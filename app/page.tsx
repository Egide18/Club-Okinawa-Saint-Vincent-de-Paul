...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      {/* ─── Chiffres clés ─── */}
      <section className="dojo-bg relative border-y border-white/10 bg-dojo-900/60 py-14">
          ...
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            {DISCIPLINES.map((d, i) => (
              <Reveal key={d.title} delay={i * 110} variant="up">
                <article className="card-hover h-full rounded-2xl border border-white/10 bg-gradient-to-b from-white/8 to-white/[0.02] p-6">
        ...
        <div className="mx-auto grid max-w-6xl items-center gap-10 px-4 sm:px-6 lg:grid-cols-2">
          <Reveal variant="left">
            ...
            <p className="mt-4 leading-relaxed text-stone-300">
              Enfants dès 7 ans, ados et adultes : les inscriptions sont ouvertes toute l’année. Déposez votre
              dossier en ligne en quelques minutes — pièce d’identité, photo, certificat médical — et recevez
              la réponse du club.
            </p>
            <div className="mt-6 flex flex-wrap gap-3">
              <Link href="/inscription" className="btn-karate">
                Déposer ma candidature <ChevronRight className="h-4 w-4" />
              </Link>
              <Link
                href="/a-propos"
