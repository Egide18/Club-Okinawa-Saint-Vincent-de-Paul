...
function DeviceBadge({ type, model }: { type?: string; model?: string }) {
  ...
  return (
    <span className="inline-flex max-w-full items-center gap-1.5 rounded-lg bg-white/10 px-2 py-1 text-xs text-stone-300 ring-1 ring-white/10">
...
export default function DashboardPage() {
  ...
  return (
    <div className="themed min-h-screen bg-dojo-950">
      {/* Barre supérieure */}
          ...
          {TABS.map((t) => {
            ...
            return (
              ...
              >
                ...
                {badge > 0 && (
                  <span className="absolute -right-1 -top-1 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-karate-gold px-1 text-[11px] font-bold text-dojo-950">
                    {badge}
                  </span>
                )}
              </button>
          ...
          <ul className="mt-3 space-y-2 text-sm">
            {inscriptions.slice(0, 5).map((i) => (
              <li key={i.id} className="flex items-center justify-between rounded-xl bg-black/30 px-3 py-2">
                <span className="truncate text-stone-200">{i.nom} {i.prenom} <span className="text-stone-500">• {i.code || i.categorie || '—'}</span></span>
...
function InscriptionsManager({ items, setItems, notify }: {
  ...
  const filtered = items.filter((i) =>
    ...
    (!search || `${i.nom} ${i.postnom} ${i.prenom} ${i.telephone} ${i.code}`.toLowerCase().includes(search.toLowerCase()))
  );
    ...
    <div>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
        <div className="relative flex-1">
          ...
          <input className="input-dark pl-10" placeholder="Rechercher nom, code à 8 caractères, téléphone…" value={search} onChange={(e) => setSearch(e.target.value)} />
        </div>
            ...
            {filtered.map((i) => (
              <tr key={i.id} className="border-b border-white/5 transition hover:bg-white/5">
                <td className="px-4 py-3">
                  {i.code ? (
                    <a href={`/pre-inscription/${i.code}`} target="_blank" rel="noopener noreferrer" title="Ouvrir la fiche officielle"
                      className="inline-block rounded-lg bg-blue-600/15 px-2.5 py-1 font-mono text-sm font-black tracking-[0.15em] text-blue-400 ring-1 ring-blue-500/40 transition hover:bg-blue-600/30">
                      {i.code}
                    </a>
                ...
                <td className="px-4 py-3">
                  ...
                  <p className="text-xs text-stone-500">{[i.telephone, i.categorie].filter(Boolean).join(' • ') || (i.age < 18 ? 'Mineur(e)' : 'Majeur(e)')}</p>
                </td>
                <td className="px-4 py-3 text-stone-300">{i.age} ans<br /><span className="text-xs text-stone-500">{i.age < 18 ? 'Mineur' : 'Majeur'}</span></td>
        ...
        {selected && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ y: 30, scale: 0.98 }} animate={{ y: 0, scale: 1 }} exit={{ y: 20 }}
              ...
              className="keep-dark mx-auto my-8 w-full max-w-2xl rounded-3xl border border-white/15 bg-dojo-900 p-6 sm:p-8">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <h3 className="font-display text-3xl tracking-wide text-white">DOSSIER DE PRÉ-INSCRIPTION</h3>
                  <p className="mt-1 text-sm text-stone-400">
                    {selected.nom} {selected.postnom} {selected.prenom} — {selected.age} ans ({selected.age < 18 ? 'mineur(e)' : 'majeur(e)'})
                  </p>
                  {selected.code && (
                    <a href={`/pre-inscription/${selected.code}`} target="_blank" rel="noopener noreferrer"
                      className="mt-2 inline-flex items-center gap-2 rounded-xl bg-blue-600/15 px-3 py-1.5 font-mono text-lg font-black tracking-[0.2em] text-blue-300 ring-1 ring-blue-500/40 transition hover:bg-blue-600/30">
                      {selected.code} <ExternalLink className="h-4 w-4" />
                    </a>
                  )}
                </div>
              ...
              <div className="mt-5 flex flex-col gap-4 sm:flex-row">
                ...
                <div className="shrink-0">
                  <div className="h-44 w-36 overflow-hidden rounded-2xl border border-white/15 bg-black/30">
                    {selected.photo ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img src={selected.photo} alt="Photo du candidat" className="h-full w-full object-cover" />
                    ) : (
                      <div className="flex h-full items-center justify-center text-xs text-stone-500">Sans photo</div>
                    )}
                  </div>
                  {selected.photo && (
                    <a href={selected.photo} target="_blank" rel="noopener noreferrer" className="mt-1 inline-flex items-center gap-1 text-xs font-bold text-karate-gold hover:underline">
                      Agrandir <ExternalLink className="h-3 w-3" />
                    </a>
                  )}
                </div>
                {/* Identité */}
                <div className="grid flex-1 gap-2 text-sm sm:grid-cols-2">
                  <Info k="Nom" v={selected.nom} />
                  <Info k="Post-nom" v={selected.postnom || '—'} />
                  <Info k="Prénom" v={selected.prenom} />
                  <Info k="Naissance" v={`${selected.dateNaissance} (${selected.age} ans)`} />
              ...
              {/* Responsable / urgence */}
              {(selected.responsable || selected.urgence) && (
                <>
                  <h4 className="mt-6 font-display text-xl tracking-widest text-karate-gold">
                    {selected.responsable ? 'RESPONSABLE LÉGAL' : 'CONTACT D’URGENCE'}
                  </h4>
                  <div className="mt-3 grid gap-2 text-sm sm:grid-cols-2">
                    <Info k="Nom" v={(selected.responsable || selected.urgence)!.nom} />
                    <Info k="Prénom" v={(selected.responsable || selected.urgence)!.prenom} />
                    <Info k="Adresse" v={(selected.responsable || selected.urgence)!.adresse} />
                    <Info k="Téléphone" v={(selected.responsable || selected.urgence)!.telephone} />
                  </div>
                </>
              )}

              {/* Ancien format : pièces justificatives (compatibilité) */}
              {selected.files && (
                <>
                  <h4 className="mt-6 font-display text-xl tracking-widest text-karate-gold">PIÈCES JUSTIFICATIVES (ANCIEN DOSSIER)</h4>
                  <ul className="mt-3 space-y-2">
                    {Object.entries(FILE_LABELS).map(([key, label]) => {
                      const fileUrl = (selected.files as Record<string, string | undefined> | undefined)?.[key];
                      return (
              ...
              <div className="mt-6 flex flex-col gap-2 sm:flex-row">
                {selected.code && (
                  <a href={`/pre-inscription/${selected.code}`} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-1.5 rounded-xl bg-blue-700 px-4 py-3 text-sm font-bold text-white hover:bg-blue-600">
                    <FileText className="h-4 w-4" /> Fiche officielle
                  </a>
                )}
        ...
        {open ? (
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              ...
              <span className="rounded-lg bg-white/10 px-2 py-1 text-xs text-stone-400 ring-1 ring-white/10">IP : {open.ip || '?'}</span>
              <span className="rounded-lg bg-white/10 px-2 py-1 text-xs text-stone-400 ring-1 ring-white/10">{fmtDate(open.createdAt)}</span>
            </div>
