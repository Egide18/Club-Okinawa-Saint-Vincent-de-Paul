...
import { Menu, X, Swords, Sun, Moon } from 'lucide-react';
...
function ThemeToggle({ compact = false }: { compact?: boolean }) {
  ...
  return (
    <button
      ...
      className={`flex items-center justify-center rounded-xl border border-white/15 bg-white/5 text-stone-200 transition hover:border-karate-gold/60 hover:text-karate-gold ${
...
export default function Navbar() {
  ...
  return (
    <header
      className={`${scrolled || open ? 'themed bg-dojo-950/90 shadow-lg shadow-black/40 backdrop-blur-xl' : 'bg-gradient-to-b from-black/70 to-transparent'} print-hidden fixed inset-x-0 top-0 z-50 transition-all duration-300`}
    >
      <nav className="mx-auto flex h-[72px] max-w-7xl items-center justify-between px-4 sm:px-6">
        ...
        <ul className="hidden items-center gap-1 lg:flex">
          ...
          <li className="ml-1">
            <Link href="/pre-inscription" className="btn-karate !px-5 !py-2.5 text-sm">
              Rejoindre le club
            </Link>
          </li>
        </ul>
      ...
      >
        <ul className="space-y-1 px-4 py-4">
          ...
          <li className="pt-2">
            <Link href="/pre-inscription" className="btn-karate w-full">
              Rejoindre le club
            </Link>
          </li>
        </ul>
